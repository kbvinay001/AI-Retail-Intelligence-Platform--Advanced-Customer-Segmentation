...
# For Google Colab, install these packages first:
# !pip install plotly scikit-learn pandas numpy matplotlib seaborn
...
class RetailIntelligencePlatform:
    ...
    def generate_synthetic_data(self, n_customers=1000, n_transactions=5000):
        """
        Generate realistic retail transaction data with various customer patterns
        """
        ...
        # Generate customer acquisition dates
        ...
        for i in range(n_transactions):
            # Select customer with weighted probability (some customers shop more)
            ...
            # Generate transaction date after acquisition
            days_since_acq = (datetime.now() - acq_date).days
            if days_since_acq <= 0:
                days_since_acq = 1
            transaction_date = acq_date + timedelta(days=np.random.randint(0, days_since_acq))

            # Generate purchase behavior based on customer patterns
            if customer_id <= n_customers * 0.1:  # 10% High-value customers
                base_amount = np.random.normal(200, 50)
                quantity = np.random.randint(3, 8)
                frequency_modifier = 3  # Shop more frequently
            elif customer_id <= n_customers * 0.3:  # 20% Medium-value customers
                base_amount = np.random.normal(100, 30)
                quantity = np.random.randint(2, 5)
                frequency_modifier = 2
            elif customer_id <= n_customers * 0.7:  # 40% Regular customers
                base_amount = np.random.normal(60, 20)
                quantity = np.random.randint(1, 4)
                frequency_modifier = 1
            else:  # 30% Low-value/Bargain hunters
            ...
            # Select product category based on customer demographics
    ...
    def calculate_rfm_metrics(self):
        """
        Calculate RFM (Recency, Frequency, Monetary) metrics with advanced features
        """
        ...
        rfm = self.transaction_data.groupby('customer_id').agg({
            ...
            'discount_applied': 'mean',  # Average discount usage
        ...
        rfm['purchase_frequency_rate'] = rfm['frequency'] / (rfm['purchase_span'] + 1)  # Avoid division by zero
    ...
    def perform_advanced_segmentation(self):
        """
        Perform multiple clustering algorithms with automatic optimization
        """
        ...
        # Use K-means as primary segmentation
    ...
    def analyze_segments(self):
        """
        Analyze and characterize customer segments with business insights
        """
        ...
        # Add segment names to main data
    ...
    def detect_anomalies(self):
        """
        Detect unusual customer behavior patterns using Isolation Forest
        """
        print("🔍 Detecting customer behavior anomalies...")

        # Use the same features as in perform_advanced_segmentation
        features = self.models['feature_names']  # Fixed: Use stored feature names
    ...
    def create_comprehensive_dashboard(self):
        ...
        # 6. Monthly Revenue by Segment (simulated)
        months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']
        for segment_name in segment_counts.index[:3]:  # Top 3 segments
    ...
    def _create_advanced_visualizations(self):
        ...
        segment_metrics = self.segments['analysis'][['segment_name', 'monetary_total_mean',
                                                    'frequency_mean', 'estimated_clv_mean',
                                                    'loyalty_score_mean', 'consistency_score_mean']]
        ...
        categories = ['Monetary Value', 'Frequency', 'Customer Lifetime Value',
                     'Loyalty Score', 'Consistency Score']
    ...
    def generate_comprehensive_report(self):
        """
        Generate a detailed business report with actionable insights
        """
...
# Example usage of the RetailIntelligencePlatform
if __name__ == "__main__":
    ...
    print(segment_analysis[['segment_name', 'customer_id_count', 'monetary_total_mean',
                          'frequency_mean', 'estimated_clv_mean']])
